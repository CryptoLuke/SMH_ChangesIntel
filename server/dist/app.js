...
export function createApp() {
    ...
    if (isProduction && (!authUser || !authPassword)) {
        ...
        throw new Error("DASHBOARD_USER and DASHBOARD_PASSWORD must both be set in production. " +
    ...
    if (authUser && authPassword) {
        app.use(basicAuth({
            ...
            unauthorizedResponse: () => "Authentication required. Enter the DASHBOARD_USER / DASHBOARD_PASSWORD credentials when prompted.",
        }));
    }
    else {
        console.warn("WARNING: DASHBOARD_USER/DASHBOARD_PASSWORD not set — running without authentication. " +
    ...
    app.get("*", (_req, res) => {
        res.sendFile(path.join(clientDist, "index.html"), (err) => {
            if (err)
                res.status(404).send("Frontend not built — run npm run build in client/ first.");
        });
    });
    return app;
}
