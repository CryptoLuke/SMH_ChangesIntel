 ...
 */
function loadConnectionFromEnv() {
    ...
    if (!baseUrl || !clientId || !clientSecret) {
        throw new Error("Set ISC_BASE_URL, ISC_CLIENT_ID, and ISC_CLIENT_SECRET before running.");
    }
...
async function run() {
    ...
    const scope = process.env.ISC_SCOPE?.split(",") ?? DEFAULT_SCOPE;
            ...
            if (baseline) {
                ...
                if (lookbackDays != null) {
                    ...
                    if (actualGapDays < lookbackDays - 0.01) {
                        console.log(`  Note: requested ${lookbackDays}d lookback for ${objectType}, but only ${actualGapDays.toFixed(1)}d of history exists — using earliest available snapshot.`);
                    }
                }
            }
        }
